"""
Kafka CSV to Delta Lake - Batch CSV Consumer

This script reads CSV-formatted messages from Kafka and processes them into Delta Lake
in batch mode (processes all messages and stops).

Usage:
    python kafka_csv_to_delta.py --topic batch-csv-topic --schema kafka_delta_batch --table iot_events
"""
...
config_file = "spark_config_delta.py"
...
def parse_csv_value(df):
    ...
    """
    # Define the schema for CSV columns
    ...
    # Split by comma and create struct
...
def process_batch_csv_to_delta(spark, db_schema, table_name, topic, bootstrap_servers):
    """
    Process CSV messages from Kafka in batch mode and write to Delta Lake.
    ...
    try:
        print(f"\n{'='*70}")
        print(f"BATCH CSV TO DELTA LAKE")
        print(f"{'='*70}")
        print(f"Topic: {topic}")
        ...
        # Write to Delta Lake
        print(f"\nWriting to Delta Lake table at: {table_path}")
        ...
        try:
            ...
            # Optimize table (only if table is registered)
            print("Optimizing Delta table...")
            optimize_start = time.time()
            spark.sql(f"OPTIMIZE {db_schema}.{table_name}")
            ...
            print("You can read it directly using: spark.read.format('delta').load('" + table_path + "')")
        ...
        if metrics.get('optimize_time', 0) > 0:
            print(f"  Optimize:        {metrics['optimize_time']:>8.2f} seconds ({metrics['optimize_time']/metrics['total_time']*100:>5.1f}%)")
...
def main():
    parser = argparse.ArgumentParser(
        description='Process CSV messages from Kafka and write to Delta Lake',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Process CSV from Kafka to Delta Lake
  python kafka_csv_to_delta.py --topic batch-csv-topic --schema kafka_delta_batch --table iot_events
  
  # Custom Kafka servers
  python kafka_csv_to_delta.py --topic batch-csv-topic --schema kafka_delta_batch --table iot_events --bootstrap-servers localhost:9092
        """
    )
