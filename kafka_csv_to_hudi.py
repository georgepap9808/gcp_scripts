"""
Kafka CSV to Apache Hudi - Batch CSV Consumer with CoW/MoR Support

This script reads CSV-formatted messages from Kafka and processes them into Apache Hudi
in batch mode. Supports both Copy-on-Write (CoW) and Merge-on-Read (MoR) table types.

Usage:
    python kafka_csv_to_hudi.py --topic batch-csv-topic --schema kafka_hudi_batch --table iot_events --table-type COPY_ON_WRITE
    python kafka_csv_to_hudi.py --topic batch-csv-topic --schema kafka_hudi_batch --table iot_events --table-type MERGE_ON_READ
"""
...
config_file = "spark_config_hudi.py"
...
def get_hudi_options(table_type, schema_name, table_name, primary_key='uuid', precombine_field='ts'):
    """
    Get Hudi write options based on table type (CoW or MoR).
    
    Args:
        table_type: 'COPY_ON_WRITE' or 'MERGE_ON_READ'
        schema_name: Database schema name
        table_name: Table name
        primary_key: Record key field (default: 'uuid')
        precombine_field: Precombine field (default: 'ts')
    
    Returns:
        dict: Hudi write options
    """
    base_options = {
        'hoodie.table.name': f"{schema_name}_{table_name}",
...
def process_batch_csv_to_hudi(spark, db_schema, table_name, topic, bootstrap_servers, 
                              table_type='COPY_ON_WRITE', primary_key='uuid'):
    """
    Process CSV messages from Kafka in batch mode and write to Apache Hudi.
    ...
    try:
        print(f"\n{'='*70}")
        print(f"BATCH CSV TO APACHE HUDI ({table_type})")
        print(f"{'='*70}")
        print(f"Topic: {topic}")
        ...
        hudi_options = get_hudi_options(table_type, db_schema, table_name, primary_key)
        ...
        # Write to Hudi table
        print(f"\nWriting to Hudi table at: {table_path}")
        ...
        # Try to register table in Hive metastore (only if Hive is available)
        try:
            spark.sql(f"""
                ...
                LOCATION '{table_path}'
            """)
            print(f"Table {db_schema}.{table_name} registered in metastore")
        except Exception as e:
            print(f"Note: Could not register table in metastore (Hive not available): {e}")
            print(f"Data written to: {table_path}")
            print("You can read it directly using: spark.read.format('hudi').load('" + table_path + "')")
        
        # Verify the write
        print("\nVerifying written data:")
        verify_start = time.time()
        # Use "hudi" format (registered by Hudi extension)
        verify_df = spark.read.format("hudi").load(table_path)
...
def main():
    parser = argparse.ArgumentParser(
        description='Process CSV messages from Kafka and write to Apache Hudi',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Process CSV from Kafka to Hudi (Copy-on-Write)
  python kafka_csv_to_hudi.py --topic batch-csv-topic --schema kafka_hudi_batch --table iot_events --table-type COPY_ON_WRITE
  
  # Process CSV from Kafka to Hudi (Merge-on-Read)
  python kafka_csv_to_hudi.py --topic batch-csv-topic --schema kafka_hudi_batch --table iot_events --table-type MERGE_ON_READ
  
  # Custom primary key
  python kafka_csv_to_hudi.py --topic batch-csv-topic --schema kafka_hudi_batch --table iot_events --table-type COPY_ON_WRITE --primary-key uuid
        """
    )
    ...
    parser.add_argument('--primary-key', type=str, default='uuid',
                       help='Primary key column name (default: uuid)')
